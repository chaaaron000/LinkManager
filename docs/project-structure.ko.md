...
- `ManagedLink`: 관리 중인 심볼릭 링크 한 개의 UI 모델입니다.
...
- 파일 시스템 유틸리티: 경로 확장, 경로 미러링, JSON 저장/로드, 디렉터리 이동, 심볼릭 링크 판별.
...
- `import_existing_link`: 기존 디렉터리 심볼릭 링크를 관리 목록에 추가합니다.
